...
# 2.IWDG独立看门狗
# 2.1 IWDG框图
![](./images/12.1-IWDG框图.png)

# 2.2IWDG键寄存器
- 键寄存器本质上是控制寄存器，用于控制硬件电路的工作
- 在可能存在干扰的情况下，一般通过在整个键寄存器写入特定值来代替控制寄存器写入一位的功能，以降低硬件电路受到干扰的概率
![](./images/12.1-IWDG键寄存器.png)

# 2.3 IWDG超时时间
- 超时时间： ${T}_ {IWDG}={T}_ {LSI}\times{PR预分频系数}\times(RL+1)$ 
- 其中： ${T}_ {LSI}= {1}/{{F}_{LSI}} $ 
![](./images/12.1-IWDG超时时间.png)

# 3.WWDG窗口看门狗
## 3.1 WWDG框图
![](./images/12.1-WWDG框图.png)
...
![](./images/12.1-WWDG超时时间计算.png)

# 4.IWDG和WWDG比较
![](./images/12.1-IWDG和WWDG比较.png)
