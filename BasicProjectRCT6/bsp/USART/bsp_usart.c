...
void USART1_IRQHandler(void)
{
    ...
    // 接收中断
    if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)                                       // 检查RXNE(读数据寄存器非空标志位); RXNE中断清理方法：读DR时自动清理；
    {
        if ((cnt >= U1_RX_BUF_SIZE))//||(xUSART.USART1ReceivedFlag==1// 判断1: 当前帧已接收到的数据量，已满(缓存区), 为避免溢出，本包后面接收到的数据直接舍弃．
        {
            ...
            USART_ReceiveData(USART1);                   // 读取数据寄存器的数据，但不保存．主要作用：读DR时自动清理接收中断标志；
            return;
        }
        RxTemp[cnt++] = USART_ReceiveData(USART1);      // 把新收到的字节数据，顺序存放到RXTemp数组中；注意：读取DR时自动清零中断位；
    }

    // 空闲中断, 用于配合接收中断，以判断一帧数据的接收完成
    if (USART_GetITStatus(USART1, USART_IT_IDLE) == SET)                  // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
    ...
    // 发送中断
    if (USART_GetFlagStatus(USART1, USART_FLAG_TXE))             // 检查TXE(发送数据寄存器空)、TXEIE(发送缓冲区空中断使能)
    {
        USART1->DR = U1TxBuffer[U1TxCounter++];                  // 读取数据寄存器值；注意：读取DR时自动清零中断位；
        if (U1TxCounter == U1TxCount)
            USART_ITConfig(USART1, USART_IT_TXE, DISABLE);       // 已发送完成，关闭发送缓冲区空置中断 TXEIE
    }
}
