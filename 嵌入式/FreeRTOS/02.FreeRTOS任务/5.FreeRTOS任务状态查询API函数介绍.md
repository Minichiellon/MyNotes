...
**时间统计API函数使用流程**  
1. 将宏$\color{red}{configGENERATE_RUN_TIME_STATS}$ 置1  
2. 将宏$\color{red}{configUSE_STATS_FORMATTING_FUNCTIONS}$ 置1  
3. 当将此宏$\color{red}{configGENERATE_RUN_TIME_STAT}$ 置1之后，还需要实现2个宏定义：  
    1. $\color{red}{portCONFIGURE_TIMER_FOR_RUNTIME_STATE() }$ ：用于初始化用于配置任务运行时间统计的时基定时器；注意：这个时基定时器的计时精度需高于系统时钟节拍精度的10至100倍！
    2. $\color{red}{portGET_RUN_TIME_COUNTER_VALUE()}$ ：用于获取该功能时基硬件定时器计数的计数值 。
